...
def get_point_move( point_list , move_vecter = [0,0]) :                                                    
    ...
    return new_point_list
        ...
        #第一个参数是 关键坐标点列表 ;  二 逆时针旋转角度;三 旋转中心默认为[0,0]
def get_point_smaller( point_list , shrink_ratio, shrink_center = [0,0]) :                                                    
    new_point_list = []
    for point in point_list:

        X1 = point[0]-shrink_center[0]
        Y1 = point[1]-shrink_center[1]
                                                
        X2 = X1*shrink_ratio+shrink_center[0]                       
        Y2 = Y1*shrink_ratio+shrink_center[1]       
        new_point = [X2 , Y2]
        new_point_list += [new_point]

    return new_point_list

def shrink_aug_jpg_png(image_path, shrink_ratio):
    file_type_list = ['jpg','png']        #定义可被查找的文件类型
    for file_type in file_type_list:
        ...
        if image_suffix in file_type:
            ...
            image_newfullname = image_frontname + "({})".format(shrink_ratio) + "." +image_suffix  #重命名
            width, height = image.size
            padding_img_size = int(height/(shrink_ratio))

            new_img = Image.new('RGB', (padding_img_size, padding_img_size))#创建画布
            edge = int((padding_img_size - height)/2)
            new_img.paste(image,(edge,edge))#贴入原图
            ratio = height/padding_img_size

            resize_img = new_img.resize(image.size,Image.ANTIALIAS)
            image_newfullname = os.path.join(os.path.dirname(image_path), image_newfullname)
            resize_img.save(image_newfullname)  #缩小并保存
...
def aug_shrink_img_json(image_path , shrink_ratio):
    ...
    image_newfullname = image_frontname + "({})".format(shrink_ratio) + "." +image_suffix                   #生成新图名字 
    image_newpath = os.path.join(os.path.dirname(image_path), image_newfullname)                                        #生成新图的路径
    
    shrink_aug_jpg_png(image_path , shrink_ratio)                                                                  #给原图增强  生成新图片
    ...
    json_newfullname = json_frontname + "({})".format(shrink_ratio) + "." +json_suffix                    #重命名
    ...
    shrink_center = [imageHeight/2,imageWidth/2]                                                       #旋转中心
    
    for all_label_dict in shape_list:                                                                  #遍历所有的标签 提取单个标签
        
        single_label_point = all_label_dict['points']                                                  #获得单个标签里所有关键点的（二级字典）
        new_all_point_list = get_point_smaller(single_label_point, shrink_ratio , shrink_center = shrink_center)  #将所有关键点绕 旋转中心 旋转
        all_label_dict['points'] = new_all_point_list                                                  #将新关键点坐标写入二级字典point关键字
    ######################################################################
...
def main():
    ...
    ######################################################################
    #Parameters that may need to be modified    (rotation_list)
    ...
    #     image_list = get_files(file_path, file_type)
    #     for image_path in tqdm(image_list):
    #         for shrink_rate in shrink_list:        
    #             aug_shrink_img_json(image_path , shrink_rate)
