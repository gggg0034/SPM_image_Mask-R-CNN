...
def apply_box_deltas_graph(boxes, deltas):                #对6000个预选框进行位置的修正      输入原始框以及对应修正量
    """Applies the given deltas to the given boxes.
    boxes: [N, (y1, x1, y2, x2)] boxes to update
    deltas: [N, (dy, dx, log(dh), log(dw))] refinements to apply
    """
    # Convert to y, x, h, w
    height = boxes[:, 2] - boxes[:, 0]                   #从6000个框中  算出框的高和宽  以及中心点
    width = boxes[:, 3] - boxes[:, 1]
    center_y = boxes[:, 0] + 0.5 * height
    center_x = boxes[:, 1] + 0.5 * width
    # Apply deltas
    center_y += deltas[:, 0] * height                    #给框的高宽中兴点进行回归参数的偏移修正
    center_x += deltas[:, 1] * width
    height *= tf.exp(deltas[:, 2])
    width *= tf.exp(deltas[:, 3])
    # Convert back to y1, x1, y2, x2
    y1 = center_y - 0.5 * height                         #把修正后的框参数还原成框的左上角与右下角的形式
...
def clip_boxes_graph(boxes, window):              #检查6000框的位置   查看是否越界  如果有   将出界的部分修剪掉
    """
    boxes: [N, (y1, x1, y2, x2)]
    ...
    # Clip
    y1 = tf.maximum(tf.minimum(y1, wy2), wy1)                    #查看是否越界  如果有   将出界的部分修剪掉
...
class ProposalLayer(KL.Layer):
    ...
    def call(self, inputs):
        # Box Scores. Use the foreground class confidence. [Batch, num_rois, 1]    #取得前景得分
        scores = inputs[0][:, :, 1]
        # Box deltas [batch, num_rois, 4]
        deltas = inputs[1]                                                         #取得偏移量
        deltas = deltas * np.reshape(self.config.RPN_BBOX_STD_DEV, [1, 1, 4])
        # Anchors  
        anchors = inputs[2]                                                        #取得所有框 6000个
        ...
        # for small objects, so we're skipping it.
                                                                                     #原版代码中此处应该有框数据的归一化操作
        # Non-max suppression
        def nms(boxes, scores):                              #非极大抑制筛选框    阈值可自己设定 
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    # Remove zero padding
    proposals, _ = trim_zeros_graph(proposals, name="trim_proposals")                         #把0 pading给剔除掉
    ...
    crowd_ix = tf.compat.v1.where(gt_class_ids < 0)[:, 0]                                    #把数据集中的重叠标注去除
    ...
    overlaps = overlaps_graph(proposals, gt_boxes)                                           #把roi的iou全部算出来

    # Compute overlaps with crowd boxes [proposals, crowd_boxes]
    crowd_overlaps = overlaps_graph(proposals, crowd_boxes)                                  #COCO数据集有关 功能暂不详
    ...
    positive_roi_bool = (roi_iou_max >= 0.5)                                                 #设置IOU  0.5以上为正样本
    positive_indices = tf.compat.v1.where(positive_roi_bool)[:, 0]                           #将正样本拿到手
    # 2. Negative ROIs are those with < 0.5 with every GT box. Skip crowds.
    negative_indices = tf.compat.v1.where(tf.logical_and(roi_iou_max < 0.5, no_crowd_bool))[:, 0]#设置IOU  0.5以下为负样本

    # Subsample ROIs. Aim for 33% positive
    # Positive ROIs
    positive_count = int(config.TRAIN_ROIS_PER_IMAGE *                                       #正样本66个    一共200个样本
                         config.ROI_POSITIVE_RATIO)                                          #正样本比例默认为0.33
    positive_indices = tf.random.shuffle(positive_indices)[:positive_count]
    positive_count = tf.shape(input=positive_indices)[0]
    # Negative ROIs. Add enough to maintain positive:negative ratio.
    r = 1.0 / config.ROI_POSITIVE_RATIO
    negative_count = tf.cast(r * tf.cast(positive_count, tf.float32), tf.int32) - positive_count  #负样本的个数为200-66
    negative_indices = tf.random.shuffle(negative_indices)[:negative_count]
    # Gather selected ROIs
    positive_rois = tf.gather(proposals, positive_indices)                                    #拿到正样本
    negative_rois = tf.gather(proposals, negative_indices)                                    #拿到负样本

    # Assign positive ROIs to GT boxes.
    positive_overlaps = tf.gather(overlaps, positive_indices)                                 #取得与真实框IOU最大的样本框 取得真实框的标签赋予给样本框
    ...
    deltas = utils.box_refinement_graph(positive_rois, roi_gt_boxes)                          #计算出样本框与真实框的偏移量
    deltas /= config.BBOX_STD_DEV

    # Assign positive ROIs to GT masks
    # Permute masks to [N, height, width, 1]
    transposed_masks = tf.expand_dims(tf.transpose(a=gt_masks, perm=[2, 0, 1]), -1)           #取得真实框的MASK为样本框自己用
    # Pick the right mask for each ROI
    roi_masks = tf.gather(transposed_masks, roi_gt_box_assignment)

    # Compute mask targets
    boxes = positive_rois
    if config.USE_MINI_MASK:                                                                  #拿好标签，制作MASK
    ...
    masks = tf.image.crop_and_resize(tf.cast(roi_masks, tf.float32), boxes,                    #重新制作好标签
                                     box_ids,
                                     config.MASK_SHAPE)
    ...
    return rois, roi_gt_class_ids, deltas, masks                                                #返回ROI 类别 偏移量 MASK掩码
...
class DetectionTargetLayer(KL.Layer):
    ...
    def call(self, inputs):
        proposals = inputs[0]                #将输入的input的各项属性数据拿过来
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):     #输入特征图  预选框位置   
    ...
    shared = KL.Conv2D(512, (3, 3), padding='same', activation='relu',                #共享卷积层
    ...
    return [rpn_class_logits, rpn_probs, rpn_bbox]                         #返回得分    返回类别概率   返回回归边框修正


def build_rpn_model(anchor_stride, anchors_per_location, depth):               #建立RPN模型    
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    x = PyramidROIAlign([pool_size, pool_size],                                                   #接上ROIAlign与两个1024的全连接层得到物体类别与方框回归修正
    ...
    # Classifier head
    mrcnn_class_logits = KL.TimeDistributed(KL.Dense(num_classes),                                    #得到物体类别   类别总数为物体类别加背景
                                            name='mrcnn_class_logits')(shared)
    mrcnn_probs = KL.TimeDistributed(KL.Activation("softmax"),                                        #得到类别后 再算softmax
                                     name="mrcnn_class")(mrcnn_class_logits)

    # BBox head
    # [batch, num_rois, NUM_CLASSES * (dy, dx, log(dh), log(dw))]
    x = KL.TimeDistributed(KL.Dense(num_classes * 4, activation='linear'),                            #算出边框回归修正参数
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),                 #添加反卷积层  将14*14*256变成28*28*256
                           name="mrcnn_mask_deconv")(x)
    x = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid"),               #用1*1卷积核把特征图卷成28*28*2，再用sigmiod激活  看看是不是MASK
                           name="mrcnn_mask")(x)
    return x
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        for p in rpn_feature_maps:                                     #对每一层特征层进行RPN模型的遍历得到rpn_class_logits, rpn_class, rpn_bbox
        ...
        # and zero padded.
                                                                                #proposal_count一般为6000，  不同的数据集可能不同
        proposal_count = config.POST_NMS_ROIS_TRAINING if mode == "training"\
            else config.POST_NMS_ROIS_INFERENCE
        rpn_rois = ProposalLayer(                                               #把6000个框经过非极大抑制   筛选出最后的几个框
