...
def main():
    count = os.listdir("./aug_test_in/") 
    index = 0
    for i in tqdm (range(0, len(count))):
        path = os.path.join("./aug_test_in", count[i])
        ...
        if os.path.isfile(path) and path.endswith('json'):
            ...
            PIL.Image.fromarray(img).save(osp.join(img_path, str(index).zfill(5)+'.jpg'))

            utils.lblsave(osp.join(label_path, str(index).zfill(5)+'.png'), lbl)
            PIL.Image.fromarray(lbl_viz).save(osp.join(label_viz_path, str(index).zfill(5)+'.png'))
 
            warnings.warn('info.yaml is being replaced by label_names.txt')
            info = dict(label_names=label_names)
            with open(osp.join(yaml_path, str(index).zfill(5)+'.yaml'), 'w') as f:
